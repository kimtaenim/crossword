...
const 식은 = [];   // «요즘 말» 그룹인데 요즘 안 나오는 말 (그룹 이름이 표시를 대신해 손댈 수 없다)
...
for (const g of pack.groups) {
  for (const w of g.words) {
    ...
    if (g.name === '요즘 말') {
      if (요즘수 < MIN_RECENT) 식은.push(`${w[0]}(${전체수})`);
      continue;   // 그룹 이름만으로 앞자리를 받는다 — 여기서 뗄 수가 없다
    }
...
줄('«요즘 말» 그룹인데 요즘 안 나오는 말 — 그룹 이름이 표시를 대신해 도구가 못 뗀다', 식은);
