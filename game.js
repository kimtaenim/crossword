...
function loadBank(pack, picked) {
  ...
  // 두 글자 덩어리(로봇·수술)를 둘 이상 나눠 가진 낱말도 친척이다 — 로봇수술과 수술로봇,
  // 청소로봇과 로봇청소기는 서로 품지 않지만 같은 재료로 만든 말이다
  // 짧은 쪽이 통째로(네 글자 이상) 긴 쪽에 있는 덩어리로만 이루어져야 한다 — 「리로봇」
  // 세 글자만 겹치는 요리로봇·갠트리로봇, 「사우루스」 만 같은 공룡들은 걸리지 않게
  const bigrams = w => { const s = new Set(); for (let i = 0; i + 2 <= w.length; i++) s.add(w.slice(i, i + 2)); return s; };
  const BG = new Map(names.map(w => [w, bigrams(w)]));
  for (let i = 0; i < names.length; i++) for (let j = i + 1; j < names.length; j++) {
    const a = names[i], b = names[j];
    if (a.includes(b) || b.includes(a)) { link(a, b); continue; }
    const [small, big] = a.length <= b.length ? [a, b] : [b, a];
    const bb = BG.get(big), covered = new Set();
...
function giveFirst(ws) {
  // 단어장이 "giveLone": 5 를 달면 기초 판에서 다섯 글자 넘는 낱말에 «겹치지 않는 칸» 을 하나
  // 연다. 첫 칸은 대개 다른 낱말과 겹쳐 있어 열어 줘 봐야 그 낱말로도 알 수 있는 글자였다.
  // 다른 낱말에서 올 길이 없는 칸이야말로 실마리가 필요한 곳이다.
  // 다만 «한 글자씩 가끔» — 밴드(여섯 줄)마다 하나, 겹치지 않는 칸이 제일 길게 이어진
  // 낱말(세 칸 이상)의 그 가운데 글자뿐이다. 낱말마다 열어 줬더니 너무 쉬워졌다
  const loneAt = EASY && PACK && PACK.giveLone ? PACK.giveLone : 0;
  const open = c => { if (c && (!c.ch || c.ch === c.ans)) { c.ch = c.ans; c.given = true; } };
  const pick = new Map();                    // 밴드 → 제일 길게 이어진 빈 칸 묶음
  for (const w of ws) {
    ...
    if (!loneAt || w.len < loneAt) continue;
    let best = null, run = [];
    ...
    if (!best) continue;
    const k = Math.floor(w.y / BAND), cur = pick.get(k);
    if (!cur || best.length > cur.length) pick.set(k, best);
  }
  for (const run of pick.values()) open(run[Math.floor(run.length / 2)]);
}
...
function renderClue() {
  ...
  // 갈래(경제와 금융·국제와 안보…)는 힌트 줄에 안 적는다.
  // 낱말이 천 개를 넘으면서 갈래가 어긋난 것이 눈에 띄기 시작했고,
  // 어차피 푸는 데 쓰이지도 않는다. 갈래는 단어장을 고를 때만 쓴다.
  bar.innerHTML =
    '<div class="meta">' +
...
/*
 * 줄을 걷어내면 호들갑을 떤다. 한 줄이면 색종이 한 줌, 네 줄이면 네 줌, 여섯 줄부터는
 * 색종이가 쏟아지고 꽃잎까지 떨어진다. 캔버스 하나에 그리고 다 떨어지면 지운다.
 * 움직임을 줄여 달라고 한 기기(prefers-reduced-motion)에서는 글자만 띄운다.
 */
let partyT = 0, party = null;
function celebrate(rows) {
  const el = document.getElementById('flash');
  el.classList.remove('big', 'huge');
  if (rows >= 6) { el.classList.add('huge'); flash('대단해요! ' + rows + '줄!!'); }
  else if (rows >= 4) { el.classList.add('big'); flash(rows + '줄 한꺼번에!'); }
  else flash(rows + '줄 넘김!');
  if (rows >= 4) { clearTimeout(flashT); flashT = setTimeout(() => el.classList.remove('on'), rows >= 6 ? 1600 : 1100); }
  if (matchMedia && matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  const cv = document.getElementById('party');
  if (!cv) return;
  const dpr = Math.min(2, window.devicePixelRatio || 1);
  const W2 = window.innerWidth, H2 = window.innerHeight;
  cv.width = W2 * dpr; cv.height = H2 * dpr; cv.hidden = false;
  const ctx = cv.getContext('2d'); ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  const css = getComputedStyle(document.body);
  const tone = ['--accent', '--cobalt', '--deep', '--lemon'].map(v => css.getPropertyValue(v).trim()).filter(Boolean);
  const palette = tone.concat(['#ff6b8a', '#ffd166', '#06d6a0', '#4cc9f0', '#f4a261', '#ffffff']);
  const ps = party ? party.ps : [];
  const n = Math.min(420, 28 * rows);                 // 한 줄 28장, 네 줄 112장, 여섯 줄 168장
  for (let i = 0; i < n; i++) {
    const big = rows >= 6;
    ps.push({ kind: 'c', x: Math.random() * W2, y: -10 - Math.random() * H2 * (big ? .5 : .25),
      vx: (Math.random() - .5) * (big ? 5 : 3), vy: 2 + Math.random() * (big ? 5 : 3.5),
      w: 6 + Math.random() * 6, h: 3 + Math.random() * 5, a: Math.random() * Math.PI, va: (Math.random() - .5) * .3,
      color: palette[(Math.random() * palette.length) | 0], life: 1 });
  }
  if (rows >= 6) {                                     // 꽃잎: 크고 느리고 하늘하늘
    const m = Math.min(90, 12 * (rows - 4));
    for (let i = 0; i < m; i++) ps.push({ kind: 'p', x: Math.random() * W2, y: -20 - Math.random() * H2 * .6,
      vx: (Math.random() - .5) * 1.2, vy: 1 + Math.random() * 1.6, sway: Math.random() * Math.PI * 2, sw: .02 + Math.random() * .03,
      r: 7 + Math.random() * 7, a: Math.random() * Math.PI, va: (Math.random() - .5) * .08,
      color: ['#ffb7c5', '#ffc8d6', '#ffe1e8', '#fff0f3', '#ffd6a5'][(Math.random() * 5) | 0], life: 1 });
  }
  const until = performance.now() + (rows >= 6 ? 3600 : rows >= 4 ? 2600 : 1800);
  if (party) { party.until = Math.max(party.until, until); return; }
  party = { ps, until };
  const tick = now => {
    ctx.clearRect(0, 0, W2, H2);
    const fade = Math.max(0, Math.min(1, (party.until - now) / 500));   // 끝나기 반 초 전부터 옅어진다
    let alive = 0;
    for (const q of party.ps) {
      if (q.y > H2 + 30) continue;
      alive++;
      q.x += q.vx; q.y += q.vy; q.a += q.va;
      if (q.kind === 'c') { q.vy += .06; q.vx *= .99; }
      else { q.sway += q.sw; q.x += Math.sin(q.sway) * 1.4; }
      ctx.save(); ctx.globalAlpha = fade; ctx.translate(q.x, q.y); ctx.rotate(q.a); ctx.fillStyle = q.color;
      if (q.kind === 'c') ctx.fillRect(-q.w / 2, -q.h / 2, q.w, q.h);
      else { ctx.beginPath(); ctx.ellipse(0, 0, q.r, q.r * .55, 0, 0, Math.PI * 2); ctx.fill(); }
      ctx.restore();
    }
    if (alive && now < party.until) partyT = requestAnimationFrame(tick);
    else { ctx.clearRect(0, 0, W2, H2); cv.hidden = true; party = null; }
  };
  partyT = requestAnimationFrame(tick);
}
...
function trace(kind, e) {
  ...
  bits.push(`len=${imeLen()} base=${imeBase} 꼬리=${JSON.stringify(tail)} 조합=${composing ? 1 : 0} 캡스=${CAPS ? 1 : 0} ⇧상태=${softShift} 칸=${JSON.stringify((S.cur && G.cells.get(S.cur) || {}).ch || '')}`);
...
window.addEventListener('keydown', e => {
  ...
  const k = e.key;
  if (k === 'Shift') { softShift = 1; return; }   // 화면 자판의 ⇧ 도 이렇게 온다 — 다음 글쇠 하나부터 다시 센다
  if (k === 'Backspace') {
